...
### 步驟 1：從 GitHub 匯入 Repository

登入 Vercel，進入公司或團隊 Team，新增 Project 並從 GitHub 匯入 Data Machi Repository。
...
### 步驟 2：確認 Branch、Root Directory 與 Vite

確認部署 Branch，將 Root Directory 設為 `frontend`，並檢查 Framework Preset、Build Command 與 Output Directory。
...
### 步驟 3：設定前端 Backend URL

建立 `VITE_API_BASE_URL`，將值指向 Render 的公開後端網址。
...
部署前逐欄核對 Framework Preset、Root Directory、Build Command 與 Output Directory，避免專案雖成功匯入卻無法建置。
...
依實際需求選擇 `VITE_API_BASE_URL` 套用到 Production、Preview 與 Development 的範圍。
...
### 步驟 6：部署並驗證 Production Domain

開始部署並查看 Build Log。Deployment 顯示 Ready 後，開啟 Production Domain，確認首頁可正常載入。
