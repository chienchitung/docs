...
> 📸 **操作截圖建議｜Trello API Key 與 Token 入口** 建議分成兩張圖：第一張標示取得 API Key 的入口，第二張標示產生 User Token 的授權畫面。不可顯示完整 Key、Token、Workspace 名稱或真實 Board 資訊。
...
> 📸 **操作截圖建議｜Atlassian API Token 建立流程** 截取 Atlassian 帳號的 API Token 管理頁、建立 Token 按鈕與命名欄位。建立完成後的 Token 僅會顯示一次，截圖時應完全遮蔽，不要用馬賽克保留可辨識輪廓。
