...
### 取得 Trello API Key 與 User Token

1. 進入 Trello 的開發者 API 頁面，確認目前登入的是準備用於整合的低權限帳號。
2. 找到 API Key 區塊並複製 Key。
3. 依頁面指示產生 User Token，閱讀授權範圍後再確認授權。
4. 將 API Key 與 Token 保存到密碼管理工具，稍後再填入後端環境變數。
...
### 建立 Confluence API Token

1. 進入 Atlassian 帳號的 API Token 管理頁。
2. 選擇建立新的 API Token。
3. 使用容易辨識的名稱，例如 `data-machi-readonly`。
4. 建立完成後立即複製 Token，並保存到密碼管理工具。
5. 記錄這組 Token 所屬帳號、用途與預計輪替日期。
