...
> 📸 **操作截圖建議｜匿名 Google Sheet 範例** 截取測試試算表的欄位名稱與 3–5 筆匿名資料，並標示 Sheet ID 位於網址中的位置；實際 ID 請遮蔽大部分字元。

> 📸 **操作截圖建議｜分享權限確認** 顯示試算表已分享給 Service Account Email 且權限為 Viewer，避免讀者誤以為只設定 JSON Key 就能存取。

> 📸 **操作截圖建議｜Sheets Tool 查詢結果** 截取 Data Machi 提問、工具執行狀態與可人工對照的答案，並在旁邊放上 Sheet 中對應資料，形成「原始資料—問題—答案」的驗證畫面。

> 📸 **操作截圖建議｜權限錯誤示範** 暫時移除分享權限後截取清楚的 Permission Denied 訊息，說明如何判斷是憑證有效但資源未授權。
