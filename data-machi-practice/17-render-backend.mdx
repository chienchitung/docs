...
> 📸 **操作截圖建議｜建立 Render Web Service** 依序截取 New Web Service、連接 GitHub Repository、選擇 Branch 與設定 Root Directory 的畫面。

> 📸 **操作截圖建議｜Build／Start Command 設定** 顯示 Runtime、Build Command、Start Command 與 `$PORT` 的完整設定位置，讓讀者可以逐欄核對。

> 📸 **操作截圖建議｜Environment Variables** 截取變數名稱清單與新增入口，所有值必須完全遮蔽；不要讓瀏覽器自動填入的 Secret 出現在畫面中。

> 📸 **操作截圖建議｜部署成功與 Log** 顯示 Service 狀態為 Live、公開 URL、成功啟動 Uvicorn 的 Log，以及 `/docs` 或 `/health` 正常回應。
