...
1. 登入 Render，進入公司或團隊 Workspace。
2. 選擇 **New Web Service**。
3. 連接 GitHub，並選擇 Data Machi Repository。
4. 選擇正式部署使用的 Branch。
5. 將 Root Directory 設為 `backend`。
6. 設定 Build Command 與 Start Command。
7. 建立後端需要的 Environment Variables。
8. 開始部署並查看 Build Log。
