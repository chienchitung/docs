...
登入 Google AI Studio，找到 API Key 管理入口。
...
### 步驟 2：選擇或建立 Google Cloud Project

API Key 必須隸屬於一個 Google Cloud Project。你可以建立新的教學專案，或選擇既有專案。
...
### 步驟 3：建立 Gemini API Key

選定 Project 後建立新的 API Key，並立即將 Key 複製到密碼管理工具。
...
將 API Key 放入密碼管理工具，不要貼進聊天、文件或 GitHub。

### 步驟 5：在後端建立 `.env`

在本機的 `backend` 資料夾建立 `.env`，並加入 Gemini API Key 與模型名稱。
