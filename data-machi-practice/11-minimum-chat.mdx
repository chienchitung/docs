...
> 📸 **操作截圖建議｜最小聊天成功畫面** 截取使用者輸入一個不需工具的測試問題，以及 Data Machi 成功回傳 Gemini 回答的完整畫面。

> 📸 **操作截圖建議｜瀏覽器 Network 請求** 顯示前端呼叫後端聊天 API 的 Request URL、Status Code 與 Response Preview，幫助讀者確認錯誤發生在前端、後端或模型層；Header 中的 Token 與敏感資料必須遮蔽。

> 📸 **操作截圖建議｜缺少 API Key 的錯誤狀態** 使用假環境測試清楚的錯誤訊息，讓讀者知道正確產品體驗不應是空白頁或無限轉圈。
