...
確認 Vite 前端、FastAPI 後端與 `GOOGLE_API_KEY` 都已準備完成。

### 步驟 2：送出不需工具的測試問題

在聊天輸入框輸入一個不需要外部資料的問題，例如「請用一句話說明什麼是 RAG」，並觀察 Loading 是否正常結束。
...
### 步驟 3：用 Network 檢查聊天 API

若聊天畫面沒有正常回應，從瀏覽器 Network 檢查 Request URL、Status Code 與 Response Preview。
...
### 步驟 4：測試缺少 API Key 的錯誤狀態

在測試環境暫時移除 API Key，確認前端會結束 Loading 並顯示可理解的設定錯誤。
