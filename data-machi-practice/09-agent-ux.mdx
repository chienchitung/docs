...
### 步驟 1：Fork 或複製 Repository

進入原始 GitHub Repository，依專案提供的方式選擇 **Fork** 或 **Use this template**。
...
### 步驟 2：設定 Owner、名稱與 Visibility

選擇 Repository 所屬的 Owner，輸入新的 Repository 名稱，並設定公開或私人權限。建立完成後，確認網址已切換成自己的 Repository。
...
從 `main` 建立新的開發 Branch，例如 `setup/own-data-machi`，避免直接在正式分支進行大量去識別化修改。

### 步驟 4：全域搜尋識別資訊

使用 GitHub 或 VS Code 全域搜尋品牌名稱、網址、Email、Sheet ID、Board ID 與其他硬編碼設定。
...
將原專案中的品牌、部門、正式網域與資料來源 ID 改成中性範例或環境變數。不要只修改前端名稱，也要檢查 Prompt、部署設定與測試資料。

### 步驟 6：建立 `.env.example` 並更新 `.gitignore`

建立只保留變數名稱的 `.env.example`，並確認 `.gitignore` 已排除 `.env`、Service Account JSON 與其他敏感檔案。
...
### 步驟 7：檢查 Diff 並建立 Commit

逐檔檢查修改內容，確認沒有誤刪功能、沒有加入真實 Secret，也沒有遺漏原組織資訊。完成驗收後，再建立第一個去識別化 Commit。
