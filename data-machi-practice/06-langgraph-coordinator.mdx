...
### 步驟 1：選擇或建立 Google Cloud Project

進入 Google Cloud Console，選擇既有 Project，或建立一個專門供 Data Machi 使用的新 Project。
...
在 API Library 搜尋並啟用 Google Sheets API；若後續需要依檔名搜尋或列出檔案，再視需求啟用 Google Drive API。
...
在 IAM 與管理後台建立新的 Service Account，名稱建議能清楚辨識用途，例如 `data-machi-sheets-reader`。
...
進入 Service Account 的 Keys 頁籤，新增 JSON Key。下載後請妥善保存，且不要上傳到 GitHub。
...
### 步驟 5：取得 Service Account Email

回到 Service Account 詳細頁，複製這個程式帳號的 Email。

### 步驟 6：將測試 Sheet 分享給 Service Account

開啟匿名測試試算表，把 Service Account Email 加入分享名單，權限先設定為 Viewer。
