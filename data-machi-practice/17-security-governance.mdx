...
1. GitHub Repository 移至公司 Organization，加入第二位 Owner。
2. Render 與 Vercel 移至公司 Workspace／Team。
3. Gemini 與 Sheets 移至公司 Google Cloud Project。
4. Confluence、Trello 改用公司控制的低權限整合身分。
5. Groq 改用公司 Project Key。
6. 全部驗證成功後，才撤銷個人憑證。
...
> 📸 **操作截圖建議｜GitHub Organization 與 Repository 權限** 截取 Repository 的 People／Teams 權限頁，標示至少兩位管理者與團隊角色；所有姓名、Email 與公司名稱應匿名化。

> 📸 **操作截圖建議｜雲端平台成員與角色** 分別示範 Render Workspace、Vercel Team 與 Google Cloud IAM 的成員管理畫面，重點標示 Owner、Developer、Viewer 等角色差異，不需展示真實成員資料。

> 📸 **操作截圖建議｜憑證清冊範例** 使用假資料呈現服務、Secret 名稱、存放位置、權限範圍、輪替日期與備援管理者；不可顯示完整憑證。

> 📸 **操作截圖建議｜Token 輪替前後** 以兩張圖呈現新 Secret 已加入環境、重新部署成功，以及舊 Token 被撤銷的順序，避免讀者先撤銷造成服務中斷。
