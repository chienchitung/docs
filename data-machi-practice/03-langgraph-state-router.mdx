...
> 📸 **操作截圖建議｜Repository、Branch 與 Commit** 截取 GitHub Repository 首頁，標示目前 Branch、最新 Commit 與檔案列表，幫助讀者建立版本中心的基本概念。

> 📸 **操作截圖建議｜建立 Feature Branch** 顯示從 `main` 建立新 Branch 的操作入口與命名方式，例如 `feature/add-sheets-tool`。

> 📸 **操作截圖建議｜Pull Request 與 Files changed** 顯示建立 PR、填寫說明，以及在 Files changed 中檢查 Diff 的位置；請使用匿名 Repository 與範例內容。

> 📸 **操作截圖建議｜Deployments／Actions 狀態** 截取 GitHub 上與 Vercel、Render 連動的部署檢查或 Actions 狀態，標示成功與失敗時要點進哪裡查看細節。
