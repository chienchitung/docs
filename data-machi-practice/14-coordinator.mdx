...
> 📸 **操作截圖建議｜不同問題的工具路由結果** 建議用 3–4 張小圖或一張拼圖，分別顯示 Sheets、RAG、不需工具與複合查詢的執行結果，畫面中應能看到系統選擇了哪個工具。

> 📸 **操作截圖建議｜LangGraph Trace** 若專案已有 Trace 或 Debug 畫面，截取 Coordinator → Tool → Coordinator 的節點順序與耗時；若尚未有視覺化介面，可使用結構化 Log 並加上框線說明。

> 📸 **操作截圖建議｜工具失敗時的來源標示** 模擬一個資料來源不可用的案例，顯示系統如何指出失敗的是哪個工具，而不是只回傳通用錯誤。
