...
> 📸 **操作截圖建議｜建立 Groq API Key** 截取 Groq Console 的 API Keys 頁面、建立按鈕與 Key 名稱欄位。完整 Key 必須完全遮蔽。

> 📸 **操作截圖建議｜會議功能輸入 Key 與上傳音檔** 截取 Data Machi 會議頁面中輸入 Groq Key、錄音或上傳匿名音檔的位置，讓讀者知道操作順序；不得使用真實會議檔名或逐字稿。

> 📸 **操作截圖建議｜成功轉錄結果** 使用 10–20 秒匿名測試音檔，顯示逐字稿與後續摘要結果，並清楚區分 Groq 轉錄與 Gemini 整理兩個階段。
