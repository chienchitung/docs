...
### 步驟 1：進入 Groq API Keys 頁面

登入 Groq Console，進入 API Keys 頁面。
...
### 步驟 2：建立並保存 Groq API Key

選擇建立新的 API Key，使用容易辨識的名稱，例如 `data-machi-transcription`。建立完成後立即複製並保存到密碼管理工具。
...
準備一段 10～20 秒、不含真實姓名、公司資訊或會議內容的音檔。若格式不支援，再使用 ffmpeg 轉換。

### 步驟 4：輸入 Groq Key 並上傳音檔

進入 Data Machi 的會議功能，輸入本次使用的 Groq Key，再選擇錄音或上傳匿名音檔。
...
先檢查音檔是否成功轉成文字。若此階段失敗，應優先檢查 Key、音訊格式、檔案大小與 API 回應。

### 步驟 6：確認 Gemini 摘要結果

逐字稿成功後，再確認 Gemini 是否完成摘要、議程與行動項目整理。
